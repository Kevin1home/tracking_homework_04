"""DVC-стадия 4: финальная оценка дообученной модели на Test_2."""
import json
...
def main():
    ...
    print(f"[eval_ft] test_2 size = {len(test_ds)}")

    model = build_model(num_classes=2, pretrained=False).to(device)
    model = load_weights(model, pcfg["finetuned_model"], device)
    criterion = nn.CrossEntropyLoss()

    loss, acc, f1, prec, rec = validate(model, test_loader, criterion, device)
    print(f"[eval_ft] loss={loss:.4f} acc={acc:.4f} f1={f1:.4f} "
    ...
    Path(pcfg["finetune_metrics"]).parent.mkdir(parents=True, exist_ok=True)
    with open(pcfg["finetune_metrics"], "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)
    print(f"[eval_ft] метрики -> {pcfg['finetune_metrics']}")
